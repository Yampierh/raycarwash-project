...
async def find_matching_detailers(
    ...
    if not parsed_sizes:
        raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail="At least one vehicle_size is required.")
        ...
        for raw in addon_ids.split(","):
            ...
            if raw:
                try:
                    ...
                    raise HTTPException(status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=f"Invalid addon UUID: '{raw}'.")

    service = await ServiceRepository(db).get_by_id(service_id)
    if service is None or not service.is_active:
        raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Service '{service_id}' not found or inactive.")
    ...
    if parsed_addon_ids:
        ...
        for aid in parsed_addon_ids:
            if aid not in found_ids:
                raise HTTPException(status_code=status.HTTP_404_NOT_FOUND, detail=f"Addon '{aid}' not found or inactive.")
    ...
    nearby, _ = await ProviderRepository(db).list_available(
        lat=lat, lng=lng, radius_miles=radius_miles, page=1, page_size=50,
    )
    if not nearby:
        return []
    ...
    results.sort(key=lambda r: (-(r.average_rating or 0.0), r.distance_miles or float("inf")))

    logger.info(
        "Matching | service=%s sizes=%s price=%d¢ %dmin radius=%.1fmi results=%d",
        service.name, vehicle_sizes, total_price_cents, total_duration_mins, radius_miles, len(results),
    )
    return results
