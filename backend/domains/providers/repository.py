...
class ProviderRepository:
    ...
    async def list_available(
        ...
        """
        Paginated discovery with optional Haversine geo-filter.
        Returns (rows, total) where each row is {user, profile, distance_miles}.
        NOTE: For production scale (>10k detailers) replace with PostGIS ST_DWithin.
        """
        filters = [
            Role.name == "detailer",
        ...
        if lat is not None and lng is not None:
            ...
            lng_delta = radius_miles / max(0.001, 69.0 * math.cos(math.radians(lat)))
            filters.extend([
        ...
        for user, profile in rows:
            ...
            if lat is not None and lng is not None:
                if profile.current_lat is not None and profile.current_lng is not None:
                    ...
                    continue
            enriched.append({"user": user, "profile": profile, "distance_miles": distance})

        if lat is not None and lng is not None:
            enriched.sort(key=lambda x: x["distance_miles"] or float("inf"))
        else:
            enriched.sort(key=lambda x: float(x["profile"].average_rating or 0), reverse=True)
    ...
    async def update_profile(self, user_id: uuid.UUID, fields: dict[str, Any]) -> ProviderProfile | None:
        ...
        await self._db.execute(
            ...
            .where(ProviderProfile.user_id == user_id, ProviderProfile.is_deleted.is_(False))
            .values(**fields)
        )
        await self._db.flush()
        return await self.get_profile(user_id)

    async def update_location(self, user_id: uuid.UUID, lat: float, lng: float) -> None:
        await self._db.execute(
            update(ProviderProfile)
            .where(ProviderProfile.user_id == user_id)
            .values(current_lat=lat, current_lng=lng, last_location_update=datetime.now(timezone.utc))
        )
        await self._db.flush()
