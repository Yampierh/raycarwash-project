...
# Se agrega 'update' para la consulta de eliminación lógica
...
class VehicleRepository:
    ...
    async def soft_delete(self, vehicle_id: uuid.UUID, owner_id: uuid.UUID) -> bool:
        """
        Marca un vehículo como eliminado (is_deleted = True).
        Retorna True si el vehículo fue encontrado y actualizado.
        """
        stmt = (
            update(Vehicle)
            .where(
        ...
        result = await self._db.execute(stmt)
        await self._db.commit() # Confirmar el cambio en la DB
