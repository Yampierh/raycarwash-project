...
async def calculate_surge(lat: float, lng: float, redis) -> tuple[float, int]:
    """
    Count active detailers within a 1-ring radius at resolution 7 and
    return (surge_multiplier, active_detailer_count).

    SURGE_THRESHOLDS is sorted ascending by max-count; the first threshold
    where active_count <= max gives the multiplier. The last entry is always
    (float('inf'), 1.0) so there is always a match.
    """
