...
def score(profile, client_lat: float, client_lng: float) -> float:
    """
    0.4 × (1/distance_miles) + 0.4 × rating + 0.2 × response_rate
    Returns 0 if no location data.
    """
...
async def assign(
    ...
    for profile in ranked[:MAX_CANDIDATES]:
        ...
        # Push offer to detailer's personal WS channel
        await redis.publish(
        ...
        # Wait for accept/decline via asyncio.Event
        ...
        try:
            ...
            if response == "accepted":
                ...
                return
            # declined → fall through to next candidate
            await db.execute(
