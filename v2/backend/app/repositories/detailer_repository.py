...
class DetailerRepository:
    ...
    async def update_location(
        ...
        """
        Update location columns and (v2) H3 index cells.

        Targeted UPDATE — no full ORM load. Called by the Location Worker
        after Fireball filter passes.
        """
        values: dict = {
            "current_lat":          lat,
            "current_lng":          lng,
        ...
        if h3_index_r7 is not None:
            values["h3_index_r7"] = h3_index_r7
        if h3_index_r9 is not None:
            values["h3_index_r9"] = h3_index_r9
