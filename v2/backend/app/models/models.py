...
class AppointmentStatus(str, enum.Enum):
    SEARCHING            = "searching"          # v2: system is finding a detailer
    PENDING              = "pending"
    CONFIRMED            = "confirmed"
    ...
    NO_SHOW              = "no_show"
    NO_DETAILER_FOUND    = "no_detailer_found"  # v2: assignment exhausted all candidates
...
class DetailerProfile(TimestampMixin, Base):
    ...
    # ---- v2: H3 geospatial index fields ----
    h3_index_r7: Mapped[str | None] = mapped_column(
        String(15), nullable=True,
        comment="H3 cell at resolution 7 (~5.16 km²) — used for k-ring search.",
    )
    h3_index_r9: Mapped[str | None] = mapped_column(
        String(15), nullable=True,
        comment="H3 cell at resolution 9 (~0.1 km²) — precise storage.",
    )
    response_rate: Mapped[Decimal] = mapped_column(
        Numeric(3, 2), nullable=False, default=Decimal("0.50"),
        comment="Fraction of assignment offers accepted. Used in scoring (0.0–1.0).",
    )
