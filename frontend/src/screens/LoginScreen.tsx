...
export default function LoginScreen({ navigation }: any) {
  ...
  const handleLogin = async () => {
    ...
    try {
      ...
      if (status === 401 || status === 400) {
        ...
        Alert.alert("Login Failed", "Incorrect email or password. Don't have an account? Tap Sign Up.");
      } else if (status === 429) {
        Alert.alert("Too Many Attempts", "Your account is temporarily locked. Please try again later.");
      } else if (!err.response) {
        Alert.alert("Connection Error", "Cannot reach the server. Make sure your device is on the same Wi-Fi as the backend.");
      } else {
  ...
  const handlePasskey = async () => {
    ...
    Alert.alert("Passkeys", "Passkey support requires a native FIDO2 module. Coming soon.");
  };
        ...
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
          ...
          >
            ...
            <View style={styles.altAuthRow}>
              ...
              <Button title="🔑 Passkey" onPress={handlePasskey} variant="secondary" size="md" style={{ flex: 1 }} />
            </View>
...
const styles = StyleSheet.create({
  ...
  brand: { color: "#FFFFFF", fontSize: 32, fontWeight: "900", letterSpacing: 1 },
