...
const QUICK_SERVICES = [
  { key: "full",  label: "Full Detail", icon: "car-wash",           color: "#3B82F6", desc: "Inside & out" },
  { key: "ext",   label: "Exterior",    icon: "spray",              color: "#10B981", desc: "Wash & wax" },
  { key: "int",   label: "Interior",    icon: "seat-recline-extra", color: "#8B5CF6", desc: "Deep clean" },
  { key: "head",  label: "Headlights",  icon: "car-light-high",     color: "#F59E0B", desc: "UV restore" },
];
...
const COLOR_MAP: Record<string, string> = {
  ...
  gold: "#D97706", charcoal: "#374151", pearl: "#BAE6FD", burgundy: "#881337",
  purple: "#7C3AED", teal: "#0D9488", maroon: "#7F1D1D", beige: "#D4B896",
};
...
function getFirstName(name?: string) { return name?.split(" ")[0] || "there"; }
...
function getColorDot(color = "") { return COLOR_MAP[color.toLowerCase()] ?? "#475569"; }
...
export default function HomeScreen() {
  ...
  const activeAptRef = useRef<any>(null);
    ...
    useCallback(() => {
      (async () => {
        ...
        try {
          ...
          setCompletedWashes(items.filter((a) => a.status === "completed").length);
          const active = items.find((a) => ["confirmed", "arrived", "in_progress"].includes(a.status));
          activeAptRef.current = active ?? null;
        } catch (e) {
  ...
  const activeApt = appointments.find((a) => ["confirmed", "arrived", "in_progress"].includes(a.status));
  ...
  useAppointmentSocket({
    ...
    onLocationUpdate: useCallback(
      (payload: { lat: number; lng: number; ts: string }) => {
        ...
        setLastKnownPosition({ lat: payload.lat, lng: payload.lng, heading: 0, timestamp: Date.now() });
      },
      [],
    ),
  });
  ...
  const locationLine = locLoading ? "Detecting…" : city ? `${city}, ${region}` : "Location unavailable";
      ...
      <ScrollView contentContainerStyle={styles.scroll} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.header}>
          <View style={{ flex: 1 }}>
            ...
            <Text style={styles.userName}>{getFirstName(userData?.full_name).toUpperCase()}</Text>
            <View style={styles.locationRow}>
              <Ionicons name="location-sharp" size={13} color={Colors.primary} />
              <Text style={styles.locationText}>{locationLine}</Text>
            </View>
          </View>
          <TouchableOpacity onPress={() => (navigation as any).navigate("Profile")} style={styles.avatarWrap}>
        ...
        {!weather.loading && (
          <LinearGradient
            colors={weather.isGoodForDetailing ? ["#0C2340", "#0B0F1A"] : ["#1C1917", "#0B0F1A"]}
            start={{ x: 0, y: 0 }}
            end={{ x: 1, y: 0 }}
          ...
          >
            <View style={styles.weatherLeft}>
              ...
              <View style={{ marginLeft: 12 }}>
                <Text style={styles.weatherTemp}>{weather.temperature !== null ? `${weather.temperature}°F` : "—"}</Text>
                <Text style={styles.weatherCond}>{weather.condition}</Text>
              </View>
            </View>
            <View style={styles.weatherRight}>
              <View style={[styles.weatherBadge, { backgroundColor: weather.isGoodForDetailing ? "#10B98120" : "#EF444420" }]}>
                <Ionicons
                  name={weather.isGoodForDetailing ? "checkmark-circle" : "close-circle"}
                  size={12}
                  color={weather.isGoodForDetailing ? "#10B981" : "#EF4444"}
                />
                <Text style={[styles.weatherBadgeText, { color: weather.isGoodForDetailing ? "#10B981" : "#EF4444" }]}>
                  {weather.isGoodForDetailing ? "Great day to detail!" : "Rain expected"}
                </Text>
              </View>
              <Text style={styles.weatherSub}>
                {weather.isGoodForDetailing ? "Book your service today" : "Schedule for another day"}
              </Text>
            </View>
          </LinearGradient>
        )}
        ...
        {activeApt && (
          <TouchableOpacity style={styles.activeBanner} activeOpacity={0.85}>
            <View style={[styles.activeBannerAccent, { backgroundColor: (Colors.status as any)[activeApt.status]?.text ?? Colors.primary }]} />
            <View style={styles.activeBannerBody}>
              <View style={styles.activeBannerRow}>
                <StatusBadge status={activeApt.status} showDot />
                <Text style={styles.activeBannerTime}>
                  {new Date(activeApt.scheduled_time).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true })}
                </Text>
              </View>
              ...
              {activeApt.vehicle && (
                <Text style={styles.activeBannerSub}>
                  ...
                  {activeApt.service_name ? `  ·  ${activeApt.service_name}` : ""}
                </Text>
              )}
            </View>
        ...
        {/* ── HERO CTA ────────────────────────────────────────────────────── */}
        <TouchableOpacity activeOpacity={0.88} onPress={() => navigation.navigate("SelectVehicles")}>
          <LinearGradient
          ...
          >
            <View style={styles.heroLeft}>
              <MaterialCommunityIcons name="car-wash" size={32} color="#60A5FA" />
              <View style={{ marginLeft: 14 }}>
                <Text style={styles.heroTitle}>BOOK A DETAIL</Text>
                <Text style={styles.heroSub}>Mobile detailing at your door</Text>
              </View>
            </View>
        ...
        <View style={styles.quickActions}>
          {[
            { icon: "car-outline",      label: "My Fleet",  onPress: () => (navigation as any).navigate("Vehicles") },
            { icon: "calendar-outline", label: "Schedule",  onPress: () => navigation.navigate("SelectVehicles") },
            { icon: "time-outline",     label: "History",   onPress: () => (navigation as any).navigate("Appointments") },
            { icon: "person-outline",   label: "Profile",   onPress: () => (navigation as any).navigate("Profile") },
          ].map((action) => (
        ...
        {/* ── YOUR FLEET ──────────────────────────────────────────────────── */}
        <SectionHeader
          title={`YOUR FLEET (${vehicles.length})`}
          action={{ label: "View all", onPress: () => (navigation as any).navigate("Vehicles") }}
          style={{ marginTop: 4 }}
        />
        ...
        {vehicles.length > 0 ? (
          <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ paddingRight: 20 }}>
            {vehicles.map((car) => {
              ...
              return (
                <View key={car.id} style={[styles.fleetCard, { width: FLEET_CARD_W }]}>
                  <LinearGradient colors={[`${colorHex}18`, "transparent"]} style={StyleSheet.absoluteFillObject} />
                  <View style={styles.fleetBadge}>
                    <Text style={styles.fleetBadgeText}>{(car.body_class || "SEDAN").toUpperCase()}</Text>
                  </View>
                  <View style={styles.carIconWrap}>
                    <MaterialCommunityIcons name={getCarIcon(car.body_class) as any} size={56} color={colorHex} style={{ opacity: 0.9 }} />
                  </View>
                  ...
                  <View style={styles.carMeta}>
                    <View style={styles.carMetaItem}>
                      ...
                      <Text style={styles.carMetaText}>{car.license_plate}</Text>
                    </View>
                    <View style={styles.carMetaItem}>
                      <View style={[styles.colorDot, { backgroundColor: colorHex, borderWidth: needsBorder ? 1 : 0, borderColor: "#334155" }]} />
                      <Text style={styles.carMetaText}>{car.color?.charAt(0).toUpperCase() + car.color?.slice(1)}</Text>
                    </View>
                  </View>
                  <Button
                    title="BOOK NOW"
                    onPress={() => navigation.navigate("Booking", { selectedVehicles: [car] })}
                    variant="primary"
                    size="sm"
                    fullWidth
                  />
            ...
            <TouchableOpacity
              style={[styles.fleetCard, styles.addFleetCard, { width: FLEET_CARD_W * 0.55 }]}
              onPress={() => navigation.navigate("AddVehicle")}
            >
              <MaterialCommunityIcons name="plus-circle-outline" size={32} color="#334155" />
          ...
          <EmptyState
            ...
            action={{ label: "Add Vehicle", onPress: () => navigation.navigate("AddVehicle") }}
          />
        )}
        ...
        <SectionHeader
          title="UPCOMING"
          action={upcomingApts.length > 1 ? { label: `${upcomingApts.length} scheduled`, onPress: () => {} } : undefined}
          style={{ marginTop: 28 }}
        />

        {nextApt ? (
          <View style={styles.aptCard}>
            <View style={[styles.aptAccent, { backgroundColor: (Colors.status as any)[nextApt.status]?.text ?? "#3B82F6" }]} />
            <View style={styles.aptBody}>
              <View style={styles.aptTopRow}>
                <StatusBadge status={nextApt.status} />
                <Text style={styles.aptCountdown}>{getCountdown(nextApt.scheduled_time)}</Text>
              </View>
              <View style={styles.aptMidRow}>
                <View>
                  <Text style={styles.aptDate}>
                    {new Date(nextApt.scheduled_time).toLocaleDateString("en-US", { weekday: "long", month: "short", day: "numeric" })}
                  </Text>
                  <Text style={styles.aptTime}>
                    {new Date(nextApt.scheduled_time).toLocaleTimeString("en-US", { hour: "numeric", minute: "2-digit", hour12: true })}
                  </Text>
                </View>
              ...
              {(nextApt.vehicle || nextApt.service_name) && (
                <Text style={styles.aptMeta}>
                  {nextApt.vehicle ? `${nextApt.vehicle.year} ${nextApt.vehicle.make} ${nextApt.vehicle.model}` : ""}
                  {nextApt.vehicle && nextApt.service_name ? "  ·  " : ""}
          ...
          <EmptyState
            ...
            action={{ label: "BOOK NOW", onPress: () => navigation.navigate("SelectVehicles") }}
          />
        )}

        {/* ── OUR SERVICES ────────────────────────────────────────────────── */}
        <Text style={[styles.sectionTitleStandalone, { marginTop: 28, marginBottom: 14 }]}>OUR SERVICES</Text>
        <View style={styles.servicesGrid}>
          {QUICK_SERVICES.map((s) => (
            <TouchableOpacity key={s.key} style={styles.serviceCard} onPress={() => navigation.navigate("SelectVehicles")}>
        ...
        {/* ── SPECIAL OFFERS ──────────────────────────────────────────────── */}
        <Text style={[styles.sectionTitleStandalone, { marginTop: 28, marginBottom: 14 }]}>SPECIAL OFFERS</Text>
        <LinearGradient
        ...
        >
          <View style={styles.offerContent}>
            ...
            <Text style={styles.offerDesc}>$20 off any Full Detail service this season</Text>
            <Button
              title="CLAIM OFFER"
              onPress={() => navigation.navigate("SelectVehicles")}
              variant="primary"
              size="sm"
              style={{ backgroundColor: "#fff", alignSelf: "flex-start", marginTop: 4 }}
            />
          </View>
          <MaterialCommunityIcons name="ticket-percent" size={80} color="#ffffff10" style={styles.offerBgIcon} />
        </LinearGradient>
...
const styles = StyleSheet.create({
  ...
  avatar: {
    ...
    backgroundColor: "#1E3A5F", borderWidth: 2, borderColor: Colors.primary,
  ...
  onlineDot: {
    ...
    backgroundColor: "#10B981", borderWidth: 2, borderColor: "#0B0F1A",
  },

  weatherCard: {
    flexDirection: "row", alignItems: "center",
    marginHorizontal: 20, borderRadius: 18, padding: 16, marginBottom: 14,
  ...
  weatherBadge: { flexDirection: "row", alignItems: "center", gap: 5, paddingHorizontal: 10, paddingVertical: 4, borderRadius: 20 },
  ...
  activeBanner: {
    flexDirection: "row", alignItems: "center",
    marginHorizontal: 20, marginBottom: 14,
    backgroundColor: "#161E2E", borderRadius: 18,
    borderWidth: 1, borderColor: "#262F3F", overflow: "hidden",
  },
  activeBannerAccent: { width: 4, alignSelf: "stretch" },
  activeBannerBody: { flex: 1, padding: 14, gap: 6 },
  activeBannerRow: { flexDirection: "row", alignItems: "center", justifyContent: "space-between" },
  ...
  statsCard: {
    ...
    marginHorizontal: 20, borderRadius: 18, paddingVertical: 18,
    marginBottom: 14, borderWidth: 1, borderColor: "#262F3F",
  },
  ...
  quickActionIcon: {
    ...
    backgroundColor: "#161E2E", borderWidth: 1, borderColor: "#262F3F",
    alignItems: "center", justifyContent: "center",
  },
  ...
  sectionTitleStandalone: {
    ...
    letterSpacing: 0.6, paddingHorizontal: 20,
  },

  fleetCard: {
    backgroundColor: "#161E2E", borderRadius: 20, padding: 16,
    marginLeft: 20, borderWidth: 1, borderColor: "#262F3F", overflow: "hidden",
  },
  fleetBadge: {
    backgroundColor: "#1E293B", alignSelf: "flex-start",
    paddingHorizontal: 8, paddingVertical: 4, borderRadius: 8, marginBottom: 10,
  },
  fleetBadgeText: { color: "#64748B", fontSize: 9, fontWeight: "800", letterSpacing: 0.5 },
  ...
  addFleetText: { color: "#334155", fontSize: 12, fontWeight: "700", textAlign: "center" },

  aptCard: {
    flexDirection: "row", marginHorizontal: 20,
    backgroundColor: "#161E2E", borderRadius: 20,
    borderWidth: 1, borderColor: "#262F3F", overflow: "hidden",
  },
  aptAccent: { width: 4 },
  aptBody: { flex: 1, padding: 16, gap: 8 },
  aptTopRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "center" },
  aptCountdown: { color: "#475569", fontSize: 12, fontWeight: "700" },
  aptMidRow: { flexDirection: "row", justifyContent: "space-between", alignItems: "flex-end" },
  ...
  servicesGrid: { flexDirection: "row", flexWrap: "wrap", paddingHorizontal: 20, gap: 12 },
  serviceCard: {
    width: (width - 52) / 2, backgroundColor: "#161E2E",
  ...
  offerCard: { marginHorizontal: 20, borderRadius: 22, padding: 22, overflow: "hidden" },
  offerContent: { gap: 8, maxWidth: "80%" },
  offerBadge: {
    backgroundColor: "#ffffff20", alignSelf: "flex-start",
    paddingHorizontal: 10, paddingVertical: 3, borderRadius: 20,
  },
  offerBadgeText: { color: "#93C5FD", fontSize: 10, fontWeight: "800", letterSpacing: 1 },
  offerTitle: { color: "#fff", fontSize: 18, fontWeight: "900" },
  offerDesc: { color: "#93C5FD", fontSize: 13, lineHeight: 18 },
  offerBgIcon: { position: "absolute", right: -10, bottom: -10 },
});
