...
export default function AddVehicleScreen({ navigation }: any) {
  ...
  const handleVinLookup = async () => {
    ...
    try {
      ...
      Alert.alert("VIN Error", "Could not auto-fill details. Please enter manually.");
    } finally {
      setDecoding(false);
    }
  };
  ...
  const handleSave = async () => {
    ...
    if (!formData.make || !formData.model || !formData.license_plate) {
      Alert.alert("Missing Fields", "Make, Model, and License Plate are required.");
      return;
    }
    setLoading(true);
    try {
      await addVehicle({
        ...
        series: (formData as any).series || "",
        notes: (formData as any).notes || "",
      });
      Alert.alert("Success", "Vehicle saved!", [{ text: "OK", onPress: () => navigation.goBack() }]);
    } catch (error: any) {
      const detail = error.response?.data?.detail;
      const msg = Array.isArray(detail) ? `${detail[0].loc[1]}: ${detail[0].msg}` : "Server Error";
  ...
  const renderOption = ({ item }: { item: string }) => (
    <TouchableOpacity
      ...
      onPress={() => {
        ...
        if (modalType === "body") setFormData({ ...formData, body_class: item });
        setModalType(null);
      }}
    >
      <Text style={styles.optionText}>{item}</Text>
      {(formData.year === item || formData.color === item || formData.body_class === item) && (
      ...
      <ScrollView contentContainerStyle={styles.scrollContent} showsVerticalScrollIndicator={false}>
        ...
        <View style={styles.vinCard}>
          ...
          <View style={styles.vinRow}>
            ...
            />
            <TouchableOpacity style={styles.decodeBtn} onPress={handleVinLookup}>
              {decoding
        ...
        <View style={styles.form}>
          <View style={styles.row}>
            <View style={styles.field}>
              ...
              <TextInput style={styles.input} value={formData.make} autoCapitalize="characters"
                onChangeText={(t) => setFormData({ ...formData, make: t })} placeholder="Ford" placeholderTextColor="#475569" />
            </View>
            <View style={styles.field}>
              <Text style={styles.label}>MODEL</Text>
              <TextInput style={styles.input} value={formData.model} autoCapitalize="characters"
                onChangeText={(t) => setFormData({ ...formData, model: t })} placeholder="Edge" placeholderTextColor="#475569" />
            </View>
          </View>
          ...
          <View style={styles.fullField}>
            ...
            <TextInput style={styles.input} value={formData.license_plate} autoCapitalize="characters"
              onChangeText={(t) => setFormData({ ...formData, license_plate: t })} placeholder="ABC-123" placeholderTextColor="#475569" />
          </View>
        </View>
...
const styles = StyleSheet.create({
  ...
  vinCard: {
    ...
    marginBottom: 24, borderWidth: 1, borderColor: "#262F3F",
  },
  vinRow: { flexDirection: "row", gap: 10, marginTop: 10 },
  vinInput: {
    flex: 1, backgroundColor: "#0B0F1A", color: "white",
    padding: 14, borderRadius: 12, borderWidth: 1, borderColor: "#334155",
    fontSize: 14,
  },
  decodeBtn: {
  ...
  fullField: { width: "100%" },
  input: {
    backgroundColor: "#161E2E", color: "white", padding: 14,
    borderRadius: 12, borderWidth: 1, borderColor: "#262F3F", fontSize: 15,
  },
  selector: {
    backgroundColor: "#161E2E", padding: 14, borderRadius: 12,
    borderWidth: 1, borderColor: "#262F3F",
