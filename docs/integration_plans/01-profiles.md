...
> | **D** | `feat/integrations/01-profiles-chunk-d` | Rename `DetailerService → ProviderService` (alias kept), table `detailer_services → provider_services`, column `detailer_id → provider_id`. New m2m table `provider_service_categories` replaces scalar `service_category_id` (data backfilled in `m_016`, scalar column dropped). `ProviderProfile.provider_services` is the canonical relationship; `.detailer_services` survives as a Python `@property` for back-compat. Wire contract on `/api/v1/detailers/me/services` unchanged — frontend doesn't move. 8 schema tests. |
> | **E** | `feat/integrations/01-profiles-chunk-e` | Drop the `is_accepting_bookings` column (`m_017`). The attribute survives as a `@property` on `ProviderProfile` that forwards reads/writes (and constructor kwargs) to `is_active`. Two real SQL query sites (`matching/service.py`, `providers/repository.py:list_available`) and one repo `update_profile` call migrated to the column name; everything else stays untouched. JSON wire keeps `is_accepting_bookings` — mobile + web + marketing unaffected. 5 property-contract tests. |
>
> **E1 closed**. Total surface across the 5 chunks: 1 new enum, 1 new m2m model, 1 new repo (`ProviderProfilesService`), 5 new endpoints (`/api/v1/providers/profiles/*`), 1 new role (`mechanic`), 3 migrations (`m_015`/`m_016`/`m_017`), 37 new tests. Phase 2 backlog: partial-unique on `(user_id, provider_type)` for soft-delete reuse; full sweep to retire the `is_accepting_bookings` wire field once frontends migrate.
