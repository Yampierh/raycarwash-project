...
| **`23-auth-hardening.md`** | :construction: **Fase 1 + 2 + 6 complete: Session ORM/repo, Redis cache, `sid` claim, login+refresh integration, enforce flag (default OFF), UA → device parser, `/auth/sessions` rich shape with `is_current`, by-id revoke endpoint** | **Critical** | None | — (absorbe `10-authorization-layer.md`) |
