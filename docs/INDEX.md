...
| **`23-auth-hardening.md`** | :construction: **Fase 1 + 2 + 3 + 6 complete: Session ORM/repo + Redis cache + `sid` claim + enforce flag, UA → device parser, `/auth/sessions` rich shape with `is_current`, `require_permission(action,resource)` + Redis-cached permission set + invalidate-on-role-mutation** | **Critical** | None | — (absorbe `10-authorization-layer.md`) |
